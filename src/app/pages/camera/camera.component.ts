...
export class CameraComponent implements OnInit {
  ...
  async onRatioChange(event: any) {
    ...
    // if last selectedRatioInput is not equal selectedRatioInput then scan camera
    if (this.lastedRatioInput != this.selectedRatioInput) {
      this.onChangeCamera = true;
    }

    // if reselect set value resolution to null
    this.selectedResolutionInput = { label: 'Select Resolution', width: 0, height: 0 };
    console.log(this.resolution);


    // set state true if selectedRatioInput is not null
    ...
    // first time selectedRatioInput is not null then scan camera
    ...
    await navigator.mediaDevices.getUserMedia({
      ...
      async (stream) => {
        ...
        this.showVideo.nativeElement.onloadeddata = async () => {
          // if selected front camera make mirror
          if (camera.side === 'Front Camera') {
            this.showVideo.nativeElement.classList.add('scale-x-[-1]');
          } else {
            this.showVideo.nativeElement.classList.remove('scale-x-[-1]');
          }
      ...
      this.showCamera = false;
      console.log(err);
      this.nativeService.presentAlert('เกิดข้อผิดพลาดขณะเปิดกล้อง', 'เกิดข้อผิดพลาดขณะเปิดกล้องเนื่องจากไม่สามารถเปิดกล้องได้ โปรดเลือกอินพุตวิดีโอใหม่อีกครั้ง');
    }).finally(async () => {
